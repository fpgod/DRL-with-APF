...
import math
from safety_layer import get_safe_action
...
class EpuckSupervisor:
    ...
    def get_observations(self):
        ...
        for i in range(self.num_robots):
            ...
            if self.dis_col[i].min() > 0.2:
                c_temp = c[index_col]
                d_temp = d[index_col]
            else:
                c_temp = c[index_col]
                d_temp = d[index_col]
            if self.dis_epu[i].min() > 0.2:
                # a_temp = np.mean(a)
                # b_temp = np.mean(b)
                a_temp = a[index]
                b_temp = b[index]
            ...
            self.observations[i] = np.hstack([a[index], b[index], delta_x, delta_y,c_temp,d_temp,
    ...
    def apf(self):
        ...
        for i in range(self.num_robots):
            ...
            if w_temp <= 2.0 and w_temp >= -2.0:
                ...
                self.w = 0.45 * w_temp
            a_temp[i][0] = (2 * 0.05 - 0.0502 * self.w) / (2 * 0.0205)
            a_temp[i][1] = (2 * 0.05 + 0.0502 * self.w) / (2 * 0.0205)
        return a_temp
        ...
        while not done:
            ...
            for n in range(env.num_robots):
                if total_steps < random_steps:  # Take the random actions in the beginning for the better exploration
                    ...
                    if safe:
                        a[n] = get_safe_action(agent.choose_action(s[n]),s[n])
                    else:
                        a[n] = agent.choose_action(s[n])
            ...
            if done:
                ...
                np.save('C:/Users/fp/Downloads/all_obs/controllers/models/SAC_near_r_{}_number_{}.npy'.format(env_name, number),
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                if evaluate_num % 10 == 0:
                    np.save('C:/Users/fp/Downloads/all_obs/controllers/models/SAC_env_{}_number_{}.npy'.format(env_name, number), np.array(evaluate_rewards))
            total_steps += 1
