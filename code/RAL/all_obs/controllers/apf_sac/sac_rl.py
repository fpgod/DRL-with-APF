...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_width, max_action,name):
        ...
        chkpt_dir = 'D:/fp/safe_rl_icra/all_obs/controllers/apf_sac'
        self.checkpoint_file = os.path.join(chkpt_dir ,name+"_safe1")
...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim):
        ...
        self.apf_a = np.zeros((self.max_size, action_dim))

    def store(self, s, a, r, s_, dw,apf_a):
        self.s[self.count] = s
        self.a[self.count] = a
        self.r[self.count] = r
        self.s_[self.count] = s_
        self.dw[self.count] = dw
        self.apf_a[self.count] = apf_a
    ...
    def sample(self, batch_size):
        ...
        batch_apf_a = torch.tensor(self.apf_a[index], dtype=torch.float)

        return batch_s, batch_a, batch_r, batch_s_, batch_dw, batch_apf_a
...
class SAC(object):
    ...
    def learn(self, relay_buffer):
        batch_s, batch_a, batch_r, batch_s_, batch_dw,batch_apf_a= relay_buffer.sample(self.batch_size)  # Sample a batch
        batch_s = batch_s
        batch_a = batch_a
        batch_r = batch_r
        batch_dw = batch_dw
        batch_s_ = batch_s_
        batch_apf_a = batch_apf_a
        q = 0.6
        epsilon = int(np.random.rand() < q)
        with torch.no_grad():
            ...
            target_Q = batch_r + 0.99 * (1 - batch_dw) * (torch.min(target_Q1, target_Q2) - self.alpha* log_pi_) + epsilon*(batch_a-batch_apf_a)
    ...
    for _ in range(times):
        ...
        while not done:
            for i in range(env.num_robots):
                ...
                #     a[i] = env.apf()[i]
                # else:
                #     a[i] = agent.choose_action(s[i], deterministic=True)
