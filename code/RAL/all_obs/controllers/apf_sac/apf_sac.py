...
class EpuckSupervisor:
    ...
    def get_safe_action(self,action,obs,c):
        for index,c_i in enumerate(c):
            g = np.array([0.0205,0.0205])
            multipliers = [(np.dot(g, action) + c_i) / np.dot(g, g)]

            multipliers = [np.clip(x, 0, np.inf) for x in multipliers]
            correction = np.max(multipliers) * g[np.argmax(multipliers)]

            action_new = action - correction*np.power(0.1,index)
...
if __name__ =='__main__':
    ...
    random_steps = 1# 25e3  # Take the random actions in the beginning for the better exploration 25e3
    ...
    while total_steps < max_train_steps:
        ...
        apf_a = np.empty((env.num_robots,2),float)
        ep_e = 0
        eposide_reward = 0
        alpha = 0.0
        while not done:
            ...
            for n in range(env.num_robots):
                if total_steps < random_steps:  # Take the random actions in the beginning for the better exploration
                    ...
                    if safe:
                        a[n] = env.get_safe_action(agent.choose_action(s[n]),s[n],c)
                        apf_a = env.apf()
                    else:
                        a[n] = agent.choose_action(s[n])
                        apf_a = env.apf()
            ...
            alpha = (env.xGradRepPotFieldTemp-0)/(1000)*np.exp(-0.1*episode_steps)
            ...
            if done:
                ...
                np.save('D:/fp/safe_rl_icra/all_obs/controllers/SAC_near_r_{}_number_{}.npy'.format(env_name, number),
                        np.array(eposide_rewards))
            else:
                dw = False
            for j in range(env.num_robots):
                replay_buffer.store(s[j], a[j], r[j], s_[j], dw,alpha*apf_a[j])  # Store the transition
            s = s_
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                if evaluate_num % 10 == 0:
                    np.save('F:/code/mappo_webots/all_obs/controllers/models/icm_env_{}_number_{}.npy'.format(env_name, number), np.array(evaluate_rewards))
            total_steps += 1
